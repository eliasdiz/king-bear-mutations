...
export default function RectRepreLegal() {
    
    const motivada = useRef()
    const [ nombreRepre, setnombreRepre ] = useState('')
    const [ tipoDocRepre, settipoDocRepre ] = useState('')
    const [ numDocRepre, setnumDocRepre ] = useState('')
    ...
    const handleCopiar = () => {
        ...
        let data = {
            'nombre repre. legal':nombreRepre,
            'tipo Doc. repre. legal': tipoDocRepre,
            '# Doc. repre. legal':numDocRepre,
        ...
        <div className='flex h-[78vh] '>
            ...
            <div className='w-[40%] flex justify-center items-center border border-gray-300'>
                ...
                <div className='opciones'>               
                    <input 
                        value={nombreRepre ? nombreRepre : ''}
                        placeholder='nombre repre. legal'
                        className='h-[35px] w-[80%] capitalize  border border-gray-500 rounded-md p-2'
                        onChange={(e) => setnombreRepre(e.target.value)}
                    />
                    ...
                    <div className='flex justify-center items-center gap-6'>
                        <Select
                            ...
                            placeholder={tipoDocRepre !== '' ? tipoDocRepre : 'doc. repre. le'}
                            options={optionsTipoDoc}
                            onChange={(e) => settipoDocRepre(e.value)}
                            value={tipoDocRepre}
                        />

                        <input 
                            value={numDocRepre ? numDocRepre : ''}
                            placeholder='# doc. repre. legal'
                            type='number'
                            className='h-[35px] w-[50%] capitalize border border-gray-500 rounded-md p-2 text-center'
                            onChange={(e) => setnumDocRepre(e.target.value)}
                        />  
                    </div>
            ...
            >
                ...
                >
                    ...
                    <br/><br/>
                    <span>
                        Que el(la) señor(a) <span className='text-red-500'>{nombreRepre}</span>, identificado(a) 
                        con <span className='text-red-500 uppercase'>{tipoDocRepre}. </span> 
                        No. <span className='text-red-500'>{formatNumber(numDocRepre)}</span>, 
